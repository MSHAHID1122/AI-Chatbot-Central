...
from content_ingest.utils import (
    markdown_to_text,
    clean_html,
    compute_hash,
...
def load_social_csv() -> List[Dict[str, Any]]:
    if not Path(SOCIAL_FILE).exists():
        return []
    ...
    for raw in tqdm(docs, desc="Processing docs"):
        ...
        for i, chunk in enumerate(chunks):
            ...
            if len(texts) >= BATCH_SIZE:
                # embed + add to chroma
                _ = create_embeddings(texts, embeddings)  # create_embeddings uses retry; chroma.add_documents stores vectors
